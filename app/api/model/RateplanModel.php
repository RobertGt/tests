...
class RateplanModel extends BaseModel
{
    ...
    public function editTaskData(array $req = []):array
    {
        ...
        $editTerminalRatingReq->setTermStartDate($params['termStartDate']);
        $editTerminalRatingReq->setTermEndDate($params['termEndDate']);
        $editTerminalRatingReq->setRenewalMode($params['renewalMode']);
        $editTerminalRatingReq->setRenewalRatePlan($params['renewalRatePlan']);
        $requestInfo = new RequestInfo();
        $requestInfo->setBaseInfo($this->requestBase);
